...
import (
	...
	"github.com/coreos/mantle/kolaform/tfdata"
...
// regex for cleaning up test names for use in GCE
var gceNameFilterRe *regexp.Regexp

func init() {
	gceNameFilterRe = regexp.MustCompile(`[^-a-z0-9]`)
}
...
func gceNameFilter(name string, limit int) string {
	name = strings.ToLower(name)
	name = gceNameFilterRe.ReplaceAllString(name, "")
	if len(name) > limit {
		name = name[:limit]
	}
	return name
...
func (c *Cluster) makePrefix() string {
	u, err := user.Current()
	if err != nil {
		c.Fatalf("current user lookup failed: %v", err)
	}
	userName := gceNameFilter(u.Username, 10)
	testName := gceNameFilter(c.test.Name, 40)
	randSufx := make([]byte, 8)
	if _, err := rand.Read(randSufx); err != nil {
		c.Fatalf("reading random data failed: %v", err)
	}

	// instance name must be less than 63 characters, this prefix is
	// limited to 57 to leave plenty of room for trailing index number.
	return fmt.Sprintf("kola-%s-%s-%x",
		userName, testName, randSufx)
}

func (c *Cluster) writeInputs() {
	data, err := json.Marshal(tfInput{
		NamePrefix:  c.makePrefix(),
